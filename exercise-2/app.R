...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput('price.choice', label = "Price (in dollars)", min = price.range[1], 
                  max = price.range[2], value = price.range),
      ...
      sliderInput('carat.choice', label = "Carats", min = carat.range[1], max = carat.range[2],
                  value = carat.range),
      ...
      selectInput('facet.choice', label = 'Facet By', choices = c('cut',
                                                                  'clarity',
                                                                  'color'))
    ),
...
my.server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    p <- ggplot(data = filtered(), mapping = aes(x = carat, y = price, color = clarity)) +
      geom_point() +
      facet_wrap(input$facet.choice) +
